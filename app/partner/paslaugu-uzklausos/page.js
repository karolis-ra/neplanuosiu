...
export default function PartnerServiceRequestsPage() {
  ...
  useEffect(() => {
    ...
    async function loadRequests() {
      try {
        ...
        if (isMounted) {
          setErrorMsg("Nepavyko užkrauti paslaugų užklausų.");
        }
      } finally {
        if (isMounted) {
          setLoading(false);
  ...
  async function updateApprovalStatus(approvalId, bookingId, nextStatus) {
    ...
    try {
      ...
      console.error("update service approval status error:", e);
      setErrorMsg("Nepavyko atnaujinti paslaugos užklausos būsenos.");
    } finally {
      setProcessingId("");
    }
  }
      ...
      <div className="mb-[28px] flex flex-col gap-[16px] md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-[8px] ui-font text-[32px] font-semibold text-slate-900">
            Mano paslaugų rezervacijos
          </h1>
          <p className="mt-[12px] ui-font text-[15px] leading-[24px] text-slate-600">
            Čia matysite rezervacijas, kuriose pasirinktos jūsų paslaugos, ir
            galėsite jas patvirtinti arba atmesti.
          </p>
          {provider && (
      ...
      <section className="space-y-[24px]">
          ...
          {grouped.pending.length === 0 ? (
            <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-[24px] py-[28px] text-center">
              <p className="ui-font text-[15px] text-slate-600">
                Šiuo metu nėra laukiančių paslaugų užklausų.
              </p>
            </div>
          ) : (
                  ...
                  >
                    <div className="flex flex-col gap-[16px] lg:flex-row lg:items-start lg:justify-between">
                      <div className="space-y-[12px]">
                        ...
                        {(service.short_description || booking.note) && (
        ...
        <div>
          ...
          {grouped.processed.length === 0 ? (
            <div className="rounded-[28px] border border-dashed border-slate-300 bg-white px-[24px] py-[28px] text-center">
              <p className="ui-font text-[15px] text-slate-600">
                Dar nėra apdorotų paslaugų užklausų.
              </p>
            </div>
          ) : (
